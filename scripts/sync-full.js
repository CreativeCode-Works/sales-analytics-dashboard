...
// ============================================================================
// SYNC ONE CONTACT (smart — only fetch what changed)
// ============================================================================

async function syncOneContact(contact) {
  const contactId = String(contact.id);

  // ALWAYS fetch field values (1 API call — cheap, always needed)
  ...
  let changes;
  if (isNew) {
    changes = { notes: true, tags: true, automations: true, deals: true };
  } else {
  ...
  // Fetch changed data from AC, load unchanged data from Supabase
  const [contactNotes, tags, automations, deals, emailActivities, contactLogs] = await Promise.all([
    ...
    fetchContactEmailActivities(contactId),  // Always fetch — opens/clicks have no count indicator
    fetchContactLogs(contactId),             // Always fetch — needed for timeline
  ]);
  ...
  // Activities: always refresh (no reliable diff)
  ...
  // Timeline: always rebuild (email activities + contact logs always re-fetched)
  ...
  if (SINGLE_CONTACT) {
    ...
    while (true) {
      ...
      try {
        ...
        console.error(`  Page fetch error at offset ${offset}: ${e.message}`);
        pageErrors++;
        if (pageErrors >= 3) {
          console.error(`  3 consecutive page errors — stopping pagination. Last successful offset: ${offset}`);
          console.error(`  To resume, run with: --offset ${offset}`);
          errors++;
          break;
        }
        // Wait longer and try next page (skip this batch)
        console.warn(`  Waiting 30s then skipping to offset ${offset + pageSize}...`);
        await sleep(30000);
        offset += pageSize;
        continue;
      }

      pageErrors = 0; // Reset on success
      ...
      for (const contact of contacts) {
        ...
        try {
          ...
          Object.keys(stats).forEach(k => { if (typeof result[k] === 'number') stats[k] += result[k]; });
          ...
          if (totalProcessed % 10 === 0 || totalProcessed === 1) {
            ...
            console.log(`  [${elapsed}s] ${totalProcessed}/${LIMIT_CONTACTS || totalContacts} contacts (${totalNew} new) | ${rate}/min | ${requestCount} API calls | notes:${stats.notes} tags:${stats.tags} auto:${stats.automations} deals:${stats.deals} email:${stats.emailActivities} sms:${stats.sms} calls:${stats.calls} timeline:${stats.timeline}`);
          }
        } catch (e) {
