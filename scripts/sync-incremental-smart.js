#!/usr/bin/env node
/**
 * Smart Incremental Sync — pulls only contacts updated since last sync,
 * then compares existing Supabase data to determine what actually changed.
 * Only fetches relational data (notes, tags, deals, etc.) when needed.
 *
 * Designed for cron: every 15 min - node scripts/sync-incremental-smart.js
 *
 * Typical incremental run: 2-5 API calls per contact (vs 22+ in full mode)
 */
...
let requestCount = 0;
async function fetchAC(path, retries = 3) {
  requestCount++;
  ...
  if (!res.ok) {
    if (res.status === 429) {
      ...
      await sleep(2000);
      return fetchAC(path, retries);
    }
    if (retries > 0 && res.status >= 500) {
      const wait = (4 - retries) * 5000;
      console.warn(`  AC API ${res.status} on ${path} — retrying in ${wait / 1000}s (${retries} left)...`);
      await sleep(wait);
      return fetchAC(path, retries - 1);
    }
...
// ============================================================================
// CACHES (loaded from Supabase lookup tables to avoid redundant AC calls)
// ============================================================================
...
// ============================================================================
// DATA FETCHERS (same as sync-incremental-full.js)
// ============================================================================
...
// ============================================================================
// NOTE PARSING (SMS + Calls from AC notes)
// ============================================================================
...
// ============================================================================
// SMART DIFF: Check what actually changed before fetching
// ============================================================================

async function getExistingData(contactId) {
  const [notesRes, tagsRes, autosRes, dealsRes, activitiesRes, timelineRes] = await Promise.all([
    supabase.from('contact_notes').select('id', { count: 'exact', head: true }).eq('contact_id', contactId),
    supabase.from('contact_tags').select('id, tag_id', { count: 'exact' }).eq('contact_id', contactId),
    supabase.from('contact_automations').select('id, automation_id, status', { count: 'exact' }).eq('contact_id', contactId),
    supabase.from('contact_deals').select('id, modified_at', { count: 'exact' }).eq('contact_id', contactId),
    supabase.from('contact_activities').select('contact_id', { count: 'exact', head: true }).eq('contact_id', contactId),
    supabase.from('timeline_events').select('contact_id', { count: 'exact', head: true }).eq('contact_id', contactId),
  ]);
  ...
  return {
    ...
    automations: autosRes.data || [],
    automationCount: autosRes.count || 0,
    deals: dealsRes.data || [],
...
async function detectChanges(contactId, existing) {
  // Quick-check AC for counts (lightweight API calls that return metadata)
  // We fetch the list endpoints but only care about the count/IDs, not full data
  const changes = {
    notes: false,
    tags: false,
    automations: false,
    ...
    contactLogs: false,  // Only fetch if other things changed (for timeline)
  };

  // Check notes count — 1 API call
  try {
    const notesData = await fetchAC(`/notes?filters[reltype]=Subscriber&filters[relid]=${contactId}&limit=1`);
    const acNoteCount = parseInt(notesData.meta?.total || notesData.notes?.length || 0);
    if (acNoteCount !== existing.noteCount) {
      changes.notes = true;
    }
  } catch (e) { changes.notes = true; } // On error, fetch to be safe

  // Check tags — 1 API call, compare tag IDs
  try {
    const tagsData = await fetchAC(`/contacts/${contactId}/contactTags`);
    const acTagIds = new Set((tagsData.contactTags || []).map(t => String(t.tag)));
    const dbTagIds = new Set(existing.tags.map(t => t.tag_id));
    if (acTagIds.size !== dbTagIds.size || [...acTagIds].some(id => !dbTagIds.has(id))) {
      changes.tags = true;
    }
  } catch (e) { changes.tags = true; }

  // Check automations — 1 API call, compare IDs + statuses
  try {
    const autosData = await fetchAC(`/contacts/${contactId}/contactAutomations`);
    const acAutos = (autosData.contactAutomations || []);
    if (acAutos.length !== existing.automationCount) {
      changes.automations = true;
    } else {
      // Same count — check if any status changed
      const dbAutoMap = {};
      for (const a of existing.automations) dbAutoMap[a.id] = a.status;
      for (const a of acAutos) {
        const acStatus = a.status === '1' ? 'Active' : a.status === '2' ? 'Completed' : 'Stopped';
        if (dbAutoMap[String(a.id)] !== acStatus) {
          changes.automations = true;
          break;
        }
      }
    }
  } catch (e) { changes.automations = true; }

  // Check deals — 1 API call, compare count + modified dates
  try {
    const dealsData = await fetchAC(`/deals?filters[contact]=${contactId}`);
    const acDeals = dealsData.deals || [];
    if (acDeals.length !== existing.dealCount) {
      changes.deals = true;
    } else {
      // Same count — check if any deal was modified
      const dbDealMap = {};
      for (const d of existing.deals) dbDealMap[d.id] = d.modified_at;
      for (const d of acDeals) {
        if (dbDealMap[String(d.id)] !== d.mdate) {
          changes.deals = true;
          break;
        }
      }
    }
  } catch (e) { changes.deals = true; }

  // Always fetch email activities — opens/clicks can happen without
  // any other relational indicator changing
  changes.emailActivities = true;

  // Always fetch contact logs — needed for complete timeline rebuild
  // (email opens/clicks and other activities may not trigger count changes)
  changes.contactLogs = true;

  return changes;
}

// ============================================================================
// SMART SYNC: Only fetch what changed
// ============================================================================

async function syncOneContactSmart(contact) {
  const contactId = String(contact.id);

  // ALWAYS fetch field values (1 API call — cheap, always needed for contact record)
  const fieldValuesRes = await fetchAC(`/contacts/${contactId}/fieldValues`).catch(() => ({ fieldValues: [] }));
  const fieldValues = fieldValuesRes.fieldValues || [];

  // Parse and upsert the contact record
  const parsed = parseContact(contact, fieldValues);

  // Check what we already have in Supabase
  const existing = await getExistingData(contactId);

  // For brand-new contacts (no timeline), do full fetch
  const isNew = !existing.hasTimeline;

  let changes;
  let skippedChecks = false;
  if (isNew) {
    changes = { notes: true, tags: true, automations: true, deals: true, emailActivities: true, contactLogs: true };
    skippedChecks = true;
  } else {
    // Detect what actually changed (5 lightweight API calls)
    changes = await detectChanges(contactId, existing);
  }

  const changedItems = Object.entries(changes).filter(([, v]) => v).map(([k]) => k);
  const unchangedCount = Object.values(changes).filter(v => !v).length;

  // Fetch only what changed
  const [contactNotes, tags, automations, deals, emailActivities, contactLogs] = await Promise.all([
    changes.notes ? fetchContactNotes(contactId) : supabase.from('contact_notes').select('*').eq('contact_id', contactId).then(r => r.data || []),
    changes.tags ? fetchContactTags(contactId) : supabase.from('contact_tags').select('*').eq('contact_id', contactId).then(r => r.data || []),
    changes.automations ? fetchContactAutomations(contactId) : supabase.from('contact_automations').select('*').eq('contact_id', contactId).then(r => r.data || []),
    changes.deals ? fetchContactDeals(contactId) : supabase.from('contact_deals').select('*').eq('contact_id', contactId).then(r => r.data || []),
    changes.emailActivities ? fetchContactEmailActivities(contactId) : supabase.from('contact_activities').select('*').eq('contact_id', contactId).then(r => r.data || []),
    changes.contactLogs ? fetchContactLogs(contactId) : [],
  ]);

  // Deal notes — only if deals or notes changed
  let allDealNotes = [];
  if (changes.deals || changes.notes) {
    for (const deal of deals) {
  ...
  // Tag-based fallback for quiz/masterclass
  ...
  // Upsert contact record (always)
  await supabase.from('contacts').upsert(parsed, { onConflict: 'id' });

  // Only upsert relational data that changed
  if (changes.notes && allNotes.length > 0) {
  ...
  // Always rebuild timeline — email activities and contact logs are always
  // re-fetched, so the timeline should always reflect current state
  ...
  return {
    notes: allNotes.length, tags: tags.length, automations: automations.length,
    deals: deals.length, emailActivities: emailActivities.length,
    sms: smsFromNotes.length, calls: callsFromNotes.length,
    timeline: anyRelationalChange ? 1 : 0,
    changed: changedItems, skipped: unchangedCount, isNew,
  };
}
...
async function getLastSyncTime() {
  const { data } = await supabase
    .from('sync_log')
    .select('completed_at')
    .in('sync_type', ['full-expanded', 'incremental-expanded', 'incremental-smart'])
    .eq('status', 'success')
...
async function run() {
  ...
  console.log(`[${new Date().toISOString()}] Starting smart incremental sync...`);
  ...
  let totalSkippedFetches = 0;
  let totalNewContacts = 0;
  let errors = 0;
  const stats = { notes: 0, tags: 0, automations: 0, deals: 0, emailActivities: 0, sms: 0, calls: 0 };
  ...
  while (true) {
    try {
      ...
      for (const contact of contacts) {
        try {
          const result = await syncOneContactSmart(contact);
          totalProcessed++;
          totalSkippedFetches += result.skipped;
          if (result.isNew) totalNewContacts++;
          Object.keys(stats).forEach(k => { if (result[k]) stats[k] += result[k]; });

          // Track what changed across all contacts
          for (const ch of (result.changed || [])) {
            changeBreakdown[ch] = (changeBreakdown[ch] || 0) + 1;
          }
  ...
  console.log(`  Contacts processed: ${totalProcessed} (${totalNewContacts} new)`);
  console.log(`  API calls: ${requestCount}`);
  console.log(`  Skipped fetches: ${totalSkippedFetches} (data unchanged)`);
  console.log(`  Change breakdown: ${JSON.stringify(changeBreakdown)}`);
  console.log(`  Stats: ${JSON.stringify(stats)}`);
  console.log(`  Errors: ${errors}`);

  // Estimate savings
  const oldEstimate = totalProcessed * 22;
  const savings = oldEstimate > 0 ? ((1 - requestCount / oldEstimate) * 100).toFixed(0) : 0;
  console.log(`  Estimated API savings: ~${savings}% (${requestCount} vs ~${oldEstimate} old method)`);
}
